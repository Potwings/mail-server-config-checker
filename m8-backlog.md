...
작성일: 2026-07-28 · 기준: PRD v1.6, `m7-plan.md`

> **✅ 2026-07-28 M8 구현 완료** — 아래 이관 항목 1·2·3·5와 본 범위 4건 모두 반영.
> 이관 1·3·5는 원본 커밋을 그대로 적용(대상 파일이 커밋 시점 이후 무변경이라 클린 적용),
> 이관 2는 판정대로 RBL nibble 조회·미지원 존 표시만 재작성, 항목 4는 판정대로 폐기.
> eml 기반 검사(DKIM·alignment·헤더 품질)는 jDKIM 의존 때문에 `checker-mail`의 `mail.check.*`에 배치.

> 이 문서의 이관 항목은 **PRD v1.6 §4 검사 항목 표와 §9 M8 마일스톤에 반영 완료**.
> PRD는 "무엇을 검사할지"(요구사항), 이 문서는 "어디서 가져와 어떻게 재작성할지"(작업 지침)를 담는다.
